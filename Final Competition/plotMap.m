function [k, b, obstacleVerts] = plotMap(mapFile, mapLimits, figHdl)
...
for i = 1:size(mapData,1)
    ...
    for j = 1:size(mapData,2)/2-1
        if (vx(i,j+1) == 0 && vy(i,j+1) == 0)  % Last vertex in the row-- close the loop.
            b = line([vx(i,j) vx(i,1)],[vy(i,j) vy(i,1)],'LineWidth',2,'Color',[0 0 1])
            obstacleVerts{i}(j,:) = [vx(i,j) vy(i,j)];
            j = j-1;
            break
        else
            b = line([vx(i,j) vx(i,j+1)],[vy(i,j) vy(i,j+1)],'LineWidth',2,'Color',[0 0 1])
            obstacleVerts{i}(j,:) = [vx(i,j) vy(i,j)];
        end
    end
    ...
    if j == size(mapData,2)/2-1
        b = line([vx(i,j+1) vx(i,1)],[vy(i,j+1) vy(i,1)],'LineWidth',2,'Color',[0 0 1])
...
k = line([vxMin vxMin],[vyMax vyMin],'LineWidth',3,'Color',[0 0 0])
